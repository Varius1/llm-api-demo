...
Терминальный клиент для [OpenRouter API](https://openrouter.ai/) с поддержкой локальных LLM (llama.cpp и совместимых серверов), красивым выводом через Rich, бенчмарком моделей и моделью-судьёй.

## Требования

- Python 3.11+
- API ключ [OpenRouter](https://openrouter.ai/keys) (не нужен при работе с локальной моделью)
...
### Локальная LLM (llama.cpp и совместимые серверы)

По умолчанию клиент запускается в режиме **локальной модели** — без обращения к OpenRouter и без необходимости API-ключа.
...
```bash
llm-cli
# локальная модель выбрана автоматически (use_local = true в config.toml)
```

#### Переключение провайдера прямо в чате

| Команда | Действие |
|---|---|
| `/model local` | Переключиться на локальный llama.cpp сервер |
| `/model openrouter` | Переключиться на OpenRouter (без конкретной модели) |
| `/model google/gemma-2-9b-it` | Переключиться на конкретную OpenRouter-модель |

#### Конфигурация

В `~/.config/llm-cli/config.toml` автоматически создаются два новых поля:

```toml
[general]
use_local = true                              # true = локальная модель при старте
local_url = "http://127.0.0.1:8081/v1/chat/completions"
```

Установите `use_local = false`, чтобы запускаться в режиме OpenRouter по умолчанию.

#### Примеры запросов разной сложности

После запуска сервера и `llm-cli` можно отправить:
...
# 3. Сложный
Объясни разницу между TCP и UDP, приведи примеры применения каждого
```

#### Техническая заметка о совместимости

Llama.cpp (и Qwen3 в частности) требует строго одно `system`-сообщение в начале. Клиент автоматически объединяет все системные сообщения (основной промпт, память, инварианты, FSM) в одно перед отправкой на локальный сервер.
...
### Оптимизация локальной LLM — демо параметров и prompt-шаблонов

```bash
# Терминал 1 — запустить llama-server
cd ~/llama.cpp && ./build/bin/llama-server \
  ...
  --ctx-checkpoints 32 --flash-attn on --no-mmap \
  --chat-template-kwargs '{"enable_thinking": false}'

# Терминал 2 — запустить демо
python -m llm_cli --local-optimize
```

Демо отправляет один и тот же технический вопрос к локальной модели в **шести конфигурациях** и выводит сравнительные таблицы через Rich.

**Этап 1 — параметры (temperature + max_tokens):**

| Конфигурация | temperature | max_tokens | Что проверяется |
|---|---|---|---|
| До оптимизации | 0.8 | 512 | Высокая температура, непредсказуемый вывод |
| Оптимальный | 0.2 | 256 | Точность + экономия токенов |
| Креативный | 1.2 | 512 | Нестандартные формулировки |

**Этап 2 — prompt-шаблоны (при фиксированных оптимальных параметрах):**

| Шаблон | Системный промпт | Что проверяется |
|---|---|---|
| Базовый | «Ты полезный ассистент.» | Ответ без специализации |
| Специализированный | Роль эксперта + ограничения + язык | Структура и точность |
| RAG-стиль | Жёсткий шаблон с нумерованными секциями | Предсказуемый формат |

Метрики по каждой конфигурации: время ответа (мс), скорость (tokens/sec), количество prompt/output-токенов, число слов в ответе.

Дополнительные параметры:

```bash
# Указать другой URL сервера
python -m llm_cli --local-optimize --local-url http://127.0.0.1:8081/v1/chat/completions

# Указать идентификатор модели
python -m llm_cli --local-optimize --local-model-id local
```
...
src/llm_cli/
  ...
  local_optimize.py — демо оптимизации локальной LLM (параметры + prompt-шаблоны)
